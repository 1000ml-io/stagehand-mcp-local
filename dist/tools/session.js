...
const CreateSessionInputSchema = z.object({
    ...
    sessionId: z
        .string()
        .optional()
        .describe("Optional session ID to use/reuse. If not provided or invalid, a new session is created."),
});
...
async function handleCreateSession(context, params) {
    const action = async () => {
        try {
            ...
            if (params.sessionId) {
                ...
                process.stderr.write(`[tool.createSession] Attempting to create/assign session with specified ID: ${targetSessionId}\n`);
            }
            else {
            ...
            if (targetSessionId === defaultSessionId) {
                ...
                session = await sessionManager.createNewBrowserSession(targetSessionId, // Internal session ID for tracking
            ...
            if (!session ||
                ...
                !session.sessionId ||
                !session.stagehand) {
                throw new Error(`SessionManager failed to return a valid session object with actualSessionId for ID: ${targetSessionId}`);
            }
            ...
            if (isLocalMode) {
                return {
                    content: [
                        {
                            type: "text",
                            text: `Local browser session created: ${session.sessionId}`,
                        },
                    ],
                };
            }
            ...
            if (!browserbaseSessionId) {
                throw new Error("Browserbase session ID not found in Stagehand instance");
            }
            ...
            process.stderr.write(`[tool.createSession] Action failed: ${errorMessage}\n`);
...
async function handleCloseSession(context) {
    const action = async () => {
        ...
        const sessionManager = context.getSessionManager();
        try {
            const session = await sessionManager.getSession(previousSessionId, context.config, false);
            if (session && session.stagehand) {
                ...
                process.stderr.write(`[tool.closeSession] No session found for ID: ${previousSessionId || "default/unknown"}\n`);
            }
        }
        catch (error) {
            cleanupErrorMessage =
                error instanceof Error ? error.message : String(error);
            process.stderr.write(`[tool.closeSession] Error cleaning up session (ID was ${previousSessionId || "default/unknown"}): ${cleanupErrorMessage}\n`);
        }
        ...
        process.stderr.write(`[tool.closeSession] Session context reset to default. Previous context session ID was ${oldContextSessionId || "default/unknown"}.\n`);
        // Step 3: Determine the result message
        const defaultSessionId = sessionManager.getDefaultSessionId();
        if (cleanupErrorMessage && !cleanupSuccessful) {
            throw new Error(`Failed to cleanup session (session ID was ${previousSessionId || "default/unknown"}). Error: ${cleanupErrorMessage}. Session context has been reset to default.`);
        }
        ...
        let infoMessage = "No active session found to close. Session context has been reset to default.";
