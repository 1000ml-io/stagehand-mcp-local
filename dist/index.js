...
export const configSchema = z
    ...
    browserbaseApiKey: z.string().optional().describe("The Browserbase API Key to use (required for BROWSERBASE mode)"),
    ...
    advancedStealth: z
        .boolean()
        .optional()
        .describe("Use advanced stealth mode. Only available to Browserbase Scale Plan users"),
    keepAlive: z
        .boolean()
        .optional()
    ...
    server: z
        ...
        host: z
            .string()
            .optional()
            .describe("The host to bind the server to. Default is localhost. Use 0.0.0.0 to bind to all interfaces"),
    })
        .optional(),
    modelName: z
        .string()
    ...
    modelApiKey: z
        .string()
        .optional()
        .describe("API key for the custom model provider. Required when using a model other than the default gemini-2.0-flash"),
    experimental: z
        .boolean()
        .optional()
    ...
    if (data.modelName && data.modelName !== "gemini-2.0-flash") {
        return (data.modelApiKey !== undefined &&
...
export default function ({ config }) {
    ...
    const server = new McpServer({
        ...
        version: "2.3.0",
        description: "Cloud browser automation server powered by Browserbase and Stagehand. Enables LLMs to navigate websites, interact with elements, extract data, and capture screenshots using natural language commands.",
        capabilities: {
            resources: {
                subscribe: true,
                listChanged: true,
    ...
    tools.forEach((tool) => {
        if (tool.schema.inputSchema instanceof z.ZodObject) {
            server.tool(tool.schema.name, tool.schema.description, tool.schema.inputSchema.shape, async (params) => {
                try {
            ...
            console.warn(`Tool "${tool.schema.name}" has an input schema that is not a ZodObject. Schema type: ${tool.schema.inputSchema.constructor.name}`);
        }
    });
