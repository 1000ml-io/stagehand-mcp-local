...
program
    ...
    .option("--modelName <model>", "The model to use for Stagehand (default: gemini-2.0-flash)")
    .option("--modelApiKey <key>", "API key for the custom model provider (required when using custom models)")
    .option("--modelBaseUrl <url>", "Custom base URL for the model provider API (e.g., for LLM gateways)")
    ...
    if (options.port)
        ...
        await startStdioTransport(serverList, config);
});
